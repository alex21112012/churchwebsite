...
import { getDB, isAdmin, uploadToStorage, parseMultipart, mapOrg } from "../../../lib/church-api/_lib";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "PUT") {
    ...
    const { data: existing } = await db.from("church_organizations").select("*").eq("id", id).single();
  ...
  if (req.method === "DELETE") {
    if (!isAdmin(req.headers)) return res.status(401).json({ error: "Unauthorized" });
    const { error } = await db.from("church_organizations").delete().eq("id", id);
    if (error) return res.status(500).json({ error: error.message });
